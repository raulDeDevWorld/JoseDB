...
export default function Button({ data, node, focusTxt }) {
    ...
    return (
        <>
            ...
            <div class="relative z-0 w-full mt-5 group ">
                <input type="email" name="floating_email" onFocus={() => setFocus(focusTxt)} onClick={stopPropagation} autocomplete="off" onChange={handlerOnChange} ref={inputRef} id="floating_email" class="block shadow-xl  py-2.5  w-full text-sm text-gray-900 bg-transparent  px-5 border border-[#dddddd] appearance-none  focus:outline-none focus:ring-0  peer rounded-[5px]" placeholder=" " required />
            ...
            {<div className={`  border overflow-auto  shadow-2xl transition-all ${focus === focusTxt ? 'h-[80px] mb-5' : 'h-[0]'}`}>
                ...
                {data.reduce((acc, i, index) => {
                    return acc.includes(i[node]) ? acc : [...acc, i[node]]
                }, []).map((i, index) => {
